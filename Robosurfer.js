...
//Turn RoboSurfer and functions on or off
  ...
  var enable_smb_delivery_ratio_scaling = true;
         ...
         if (timeDiff_Now >= 12 || timeDiff_2Periods >= 17 || glucoseDiff_Now >= maxDeltaTick || glucoseDiff_PriorPeriod >= maxDeltaTick ) {      
                      ...
                      sensor_safety_status = "On(Current Data Gap Mins(>=12)/2 Reading Data Gap Mins(>=17)/Current Glucose Tick/Prev. Glucose Tick(>=37:)  " + timeDiff_Now + "/" + timeDiff_2Periods + "/" + round(glucoseDiff_Now,0) + "/" + round(glucoseDiff_PriorPeriod,0);
...
if (enable_Automation_1) { 
          ...
          // Baseline Nightboost settings are below, regardless of ROC.  E.g. If it's after 8p and BG > 120, Sig Max is 1.45 and SMB/UAM is +15 mins
            ...
            // Set Nightboost Threshold 1 Factors    
                  Automation_Status = Automation_1_name + " OnMax1.45";   
         ...
         // E.g. If ROC is +24mg/dl in 15 mins, Sig Max is 1.7 - 2 and SMB/UAM is +30mins
         // E.g. If ROC is +20mg/dl in  5 mins, +40 in 10 mins, or +60 in 15 minsm Sig Max is 2 - 2.5 and SMB/UAM is +45mins 
         // Once ROC levels of, reveerts to baseline Nightboost even if BG high    
             
            //Increased Rate of Change (1.6mg/dl per minute)
             if (glucoseRateOfChange_3Periods > 1.6) {
             
                //120-139 (Max: 1.7, AF 1)
                if ((myGlucose >= Automation_1_BGThreshold_1 && myGlucose < Automation_1_BGThreshold_2)) {  
                      // Set Nightboost Threshold 2 Factors    
                     Automation_Status = Automation_1_name + " OnROCMax1.7";   
            ...
            //High Rate of Change (4mg/dl per minute)
             if (glucoseRateOfChange_Now > 4 || glucoseRateOfChange_2Periods > 4 || glucoseRateOfChange_3Periods > 4) {  
                  ...
                  if ((myGlucose >= Automation_1_BGThreshold_1 && myGlucose < Automation_1_BGThreshold_2)) {  
                        ...
                        Automation_Status = Automation_1_name + " OnHighROCMax2";
                        NightBoost_Sigmoid_Min = Automation_1_minimumRatio_4;
                        NightBoost_Sigmoid_Max = Automation_1_maximumRatio_4;
                        NightBoost_Sigmoid_AF = Automation_1_adjustmentFactor_4;
                   ...
                   // 140+ ((Max: 2, AF 1)
                  if (myGlucose >= Automation_1_BGThreshold_2) {
                     // Set Nightboost Threshold 6 Factors with Acceleration    
                     Automation_Status = Automation_1_name + " On HighROCMax2.5";
...
return "Robosens Status Basal/ISF: " + round(robosens_basalFactor,2) + "(" + robosens_basal_status + ")/" + round(robosens_sigmoidFactor, 2) + "(" + robosens_sens_status +  "). dISF ratio: " + round(new_dynISF_ratio, 2) + ". ISF was/now: " + round(initial_isf, 2) + "/ " + round(profile.sens,2) + " Basal was/now: " + old_basal + "/ " + profile.current_basal + ". CR was/now: " + initial_cr + "/ " + round(profile.carb_ratio, 2) + " CSF was/now "  + round(initial_csf, 2) + "/ " + round(check_csf, 2)+ ". SMB Deliv. Ratio: " + profile.smb_delivery_ratio + " ROBOSENS: Trg/Av/%Over: 4Hr: " + target_averageGlucose_Last4Hours + "/" + round(averageGlucose_Last4Hours, 0) + "/" + round(percentageOverTarget_Last4Hours, 0) + "%" + 
" 8Hr:" + target_averageGlucose_Last8Hours + "/" + round(averageGlucose_Last8Hours, 0) + "/" + round(percentageOverTarget_Last8Hours, 0) + "%" + 
" 24Hr:" + target_averageGlucose_Last24Hours + "/" + round(averageGlucose_Last24Hours, 0) + "/" + round(percentageOverTarget_Last24Hours, 0) + "%" + " RS Adj/AF: " + round(robosens_AF_adjustment,2) + "/" + round(robosens_adjustmentFactor,2) + " RS Adj/MAX: " + round(robosens_MAX_adjustment,2) + "/" + round(robosens_maximumRatio,2) + " AUTOMATION1: " + Automation_Status + ": " + start_time.toLocaleTimeString([],{hour: '2-digit', minute:'2-digit'}) + " to " + end_time.toLocaleTimeString([],{hour: '2-digit', minute:'2-digit'}) + ". SMB Mins: "  + round(profile.maxSMBBasalMinutes, 2) + " UAM Mins: "  + round(profile.maxUAMSMBBasalMinutes, 2) + " Max COB: "  + round(profile.maxCOB, 2) + ". MinAbsorp((CI): "  + round(check_carb_absorption, 2) + "(" + profile.min_5m_carbimpact + ")"  + "Sensor Safety: " + sensor_safety_status + " TDD:" + round(past2hoursAverage, 2) + " 2week TDD:" + round(average_total_data, 2);
   }
}
